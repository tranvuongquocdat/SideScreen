...
### Fixed
- **USB device selection.** Ignore emulators and Wi-Fi devices, select a sole available USB device automatically (including with libusb), ask when several are available, retire the previous tunnel when switching, and show ADB errors and device states in Status.
