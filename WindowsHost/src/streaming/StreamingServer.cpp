...
static int32_t readBE32(const uint8_t* src) {
...
// Read a float from Little-Endian bytes (x86 is LE natively, so just memcpy)
...
bool StreamingServer::initWinsock() {
    if (m_wsaInitialized) return true;

    WSADATA wsaData;
    int result = WSAStartup(MAKEWORD(2, 2), &wsaData);
    if (result != 0) {
        printf("[StreamingServer] WSAStartup failed: %d\n", result);
        return false;
    }
...
void StreamingServer::cleanupWinsock() {
    if (m_wsaInitialized) {
        WSACleanup();
...
bool StreamingServer::createListenSocket() {
    ...
    if (m_listenSocket == INVALID_SOCKET) {
        printf("[StreamingServer] socket() failed: %d\n", WSAGetLastError());
        return false;
    }
    ...
    if (bind(m_listenSocket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR) {
        printf("[StreamingServer] bind() failed: %d\n", WSAGetLastError());
        closesocket(m_listenSocket);
        m_listenSocket = INVALID_SOCKET;
        return false;
    }
    ...
    if (listen(m_listenSocket, 1) == SOCKET_ERROR) {
        printf("[StreamingServer] listen() failed: %d\n", WSAGetLastError());
        closesocket(m_listenSocket);
        m_listenSocket = INVALID_SOCKET;
        return false;
    }
...
void StreamingServer::acceptLoop() {
    while (m_running.load()) {
        ...
        SOCKET newClient = accept(m_listenSocket,
        ...
        if (newClient == INVALID_SOCKET) {
            ...
            printf("[StreamingServer] accept() failed: %d\n", WSAGetLastError());
            continue;
        }
...
void StreamingServer::handleClient(SOCKET clientSocket) {
...
void StreamingServer::receiveLoop() {
    // Maximum incoming message size is 22 bytes (2-finger touch)
    // But we read one byte at a time for the message type, then read the rest
    uint8_t msgType;
...
void StreamingServer::closeClient() {
    ...
    if (m_clientSocket != INVALID_SOCKET) {
        // Graceful shutdown: stop sends, then close
...
bool StreamingServer::sendAll(SOCKET sock, const uint8_t* data, size_t size) {
    size_t totalSent = 0;
...
bool StreamingServer::recvAll(SOCKET sock, uint8_t* data, size_t size) {
    size_t totalRecv = 0;
    while (totalRecv < size) {
        int received = recv(sock, reinterpret_cast<char*>(data + totalRecv),
